...
`sat` is a Rust research platform for building a genuinely competitive SAT
solver and, once the Boolean engine is strong enough, an SMT solver. It now has
a proof-checked modern CDCL baseline and one targeted family on which a frozen
treatment matches the pinned reference solver's solved count. That is progress,
not a broad state-of-the-art performance claim.
...
- strict DIMACS CNF parsing;
- SAT Competition output and exit codes (`10` for SAT, `20` for UNSAT);
- streaming textual DRAT output for independently checkable UNSAT results;
...
Missing pieces are tracked candidly in [the roadmap](docs/ROADMAP.md). The
proof stream has been checked with DRAT-trim on smoke and real competition
instances. A targeted REGN comparison is now competitive with pinned Kissat
4.0.4, but the small corpus and a failed mixed-family promotion gate do not
support a general competitiveness claim. The benchmark runner also does not
yet validate every UNSAT row.
...
other than the final argument. See [benchmarking](docs/BENCHMARKING.md) before
...
## Why SAT comes before SMT

SMT is not one algorithm or one leaderboard. A competitive SMT system combines
a Boolean search engine with normalization, theory propagation, explanation,
and specialized decision procedures for selected logics. Bit-vector solving
also leans heavily on SAT after bit-blasting. We will therefore establish a
fast, trustworthy Boolean kernel first, then choose a narrow SMT track using
data rather than trying to implement every theory at once.
...
result, and an independently verified claim. See [CONTRIBUTING.md](CONTRIBUTING.md).
