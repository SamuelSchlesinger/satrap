...
- Query-specific QF_BOOL and QF_BV `get-proof` certificates whose active
  premises, fixed-width bit-blast, and canonical CNF are reconstructed
  independently before DRAT checking; SMT-LIB's empty-explicit-assumption rule
  is enforced.
...
available for QF_BOOL and QF_BV; requesting `:produce-proofs` with UF, array,
or arithmetic logics is rejected until those theories have independently
...
reconstructs scoped QF_BOOL and QF_BV queries from their original SMT-LIB
input, independently repeats the bit-vector lowering, and checks each
query-specific DRAT refutation. These generated checks are correctness
