...
| Proof | `make check-proofs` | Release SAT certificates plus query-bound QF_BOOL/QF_BV/QF_UF/QF_UFBV certificates reconstructed independently and checked by pinned DRAT-trim |
...
reconstructs active QF_BOOL, QF_BV, QF_UF, and QF_UFBV queries in an
independent Python implementation, independently repeats fixed-width
bit-vector lowering, finite ground-UF class/congruence lowering, and canonical
CNF generation, and then checks the embedded DRAT suffix. Array and arithmetic
logics remain outside that claim and are rejected when proof production is
requested.
...
RustSec. It also requires every canonical QF_BOOL, QF_BV, QF_UF, and QF_UFBV
proof-corpus file to remain present and nonempty.
